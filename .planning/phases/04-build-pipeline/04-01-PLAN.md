...
must_haves:
  ...
  artifacts:
    ...
    - path: "AGENTS.md"
      provides: "Updated project structure listing ERC8004-SKILL.md instead of ai.html"
      contains: "ERC8004-SKILL.md"
  key_links:
    - from: "package.json"
...
Output: Updated package.json dist script, updated index.html with "For Agents" box, ai.html deleted, AGENTS.md updated.
</objective>
...
<task type="auto">
  <name>Task 1: Update dist script, add "For Agents" box to index.html, and update AGENTS.md</name>
  <files>package.json, public/index.html, AGENTS.md</files>
  <action>
...
**AGENTS.md** — In the `## Project structure` code block, replace the `ai.html` line with the new filename. Change:
```
  ai.html      — machine-readable agent instructions for content signing
```
to:
```
...
Run: `grep 'ERC8004-SKILL.md' AGENTS.md` — should show the updated project structure line.
Run: `grep 'ai.html' AGENTS.md` — should return no matches.
  </verify>
  <done>package.json dist script copies ERC8004-SKILL.md (not ai.html). index.html has a visible "For Agents" section with the correct fetch URL. AGENTS.md project structure references ERC8004-SKILL.md. No references to ai.html remain in any of the three files.</done>
</task>
...
Run: `grep -r 'ai.html' public/ package.json AGENTS.md` — should return no matches.
  </verify>
...
- BUILD-04: ai.html does not exist anywhere in the project, no references remain in any file
...
4. No references to ai.html remain in any project file — verified by grep across public/, package.json, and AGENTS.md
</verification>
...
- `grep -r 'ai.html' public/ package.json AGENTS.md` returns no matches
</success_criteria>
